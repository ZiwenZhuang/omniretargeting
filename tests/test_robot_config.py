...
def test_load_robot_config_preserves_joint_pos_fitting_smplx(tmp_path: Path):
    config_path = tmp_path / "robot.json"
    config = {
        "name": "test_robot",
        "urdf_path": "robot.urdf",
        "joint_mapping": {"Pelvis": "pelvis"},
        "joint_pos_fitting_smplx": {
            "left_shoulder_joint": 0.4,
            "right_shoulder_joint": -0.4,
        },
    }
    config_path.write_text(json.dumps(config), encoding="utf-8")
    ...
    assert loaded["joint_pos_fitting_smplx"] == {
        "left_shoulder_joint": 0.4,
        "right_shoulder_joint": -0.4,
    }
...
def test_load_robot_config_requires_joint_pos_fitting_smplx_object(tmp_path: Path):
    config_path = tmp_path / "invalid_joint_pos_fitting.json"
    config = {
        "name": "broken",
        "joint_mapping": {"Pelvis": "pelvis"},
        "joint_pos_fitting_smplx": [0.1, 0.2],
    }
    config_path.write_text(json.dumps(config), encoding="utf-8")

    with pytest.raises(ValueError, match="joint_pos_fitting_smplx"):
        load_robot_config(config_path)
